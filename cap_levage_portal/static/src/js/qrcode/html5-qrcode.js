...
class Html5Qrcode {
    ...
    start(cameraIdOrConfig,
        ...
        // E-cosi - debut
        this.qrcode.doNegatif = config.doNegatif ? config.doNegatif : false;
        // E-cosi - fin
        ...
        const scanContextWithNegatif = () => {
            try {
                $this.qrcode.doNegatif = true;
                $this.qrcode.decode();
                this._possiblyUpdateShaders(/* qrMatch= */ true);
                return true;
            } catch (exception) {
        ...
        const foreverScan = () => {
            ...
            if ($this._localMediaStream) {
                ...
                // E-cosi - debut - ajout du scan par négatif en plus
                const firstScanResult = scanContext();
                let secondScanResult = false;
                if (!firstScanResult) {
                    secondScanResult = scanContextWithNegatif();
                }
                if (!firstScanResult && !secondScanResult && config.disableFlip !== true) {
                    // scan inverted context.
                    this._context.translate(this._context.canvas.width, 0);
                    this._context.scale(-1, 1);
                    const scanResult = scanContext();
                    if (!scanResult) {
                        scanContextWithNegatif();
                    }
                }
                // E-cosi - fin
            }
